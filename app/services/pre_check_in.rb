    ...
    def initialize(twilio_params)
      ...
      when patron.pending
        ...
        if parsed_name.empty?
          ...
          @response_content = "Thanks! For demonstration purposes {#patron.loyalty_card.stamps_until_full} check_in(s) will fill the card so you can see redeem it for a 'reward'."
        end
        #LoyaltyCardMailer.stamped_card(patron.sms_address, file_name_of_card).deliver_now
      else
        Rails.logger.info " #{self.class.to_s}##{__method__.to_s} check in for existing patron."
        ...
        if stamp_service.full_card
          ...
          @response_content = 'Got it!  For demonstration purposes {#patron.loyalty_card.stamps_until_full} check_in(s) will fill the card so you can see redeem it for a 'reward'.'
        end
        # LoyaltyCardMailer.stamped_card(patron.sms_address, stamp_service.file_name_of_card).deliver_now
