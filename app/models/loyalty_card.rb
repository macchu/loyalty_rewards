class LoyaltyCard < ApplicationRecord
  ...
  def stamps_until_full
    if stamp_count.nil?
      self.stamps_required
    else
      self.stamps_required - self.stamp_count
    end
  end
  ...
  def create_redemption_if_full(is_demo: false)
    ...
    if self.full? and self.redemption.nil?
      redemption = Redemption.new(loyalty_card: self, 
                                  store: self.store, 
                                  patron: self.patron, 
                                  redemption_code_type: self.store.redemption_code_type,
                                  is_demo: is_demo)
